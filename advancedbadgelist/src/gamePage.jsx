(async function() {
    ...
    async function getSVG(URL) {
        ...
        const SVG = await fetch(url).then(function(response) {
    ...
    const filterTypes = {
        ...
        compareDate: {
            ...
            get: function(badge, inputs) {
                ...
                if (targetTime == undefined) return true;
    ...
    function createBasicDropdown(dropdownGroup, id) {
        return (<select class="abl-background abl-pad-sm" id={id}>
            {dropdownGroup.options.map((item, index) => (
                <option key={index} value={item} class="abl-dropdown-option">
                    {item}
                </option>
            ))}
        </select>)
    ...
    function createBadgeListUI() {
        return (
            <div class="abl-container abl-column">
                ...
                <div class="abl-container abl-row" id="abl-ownership-container">
                    <img src="" alt="" class="abl-icon abl-icon--lg" id="abl-viewing-img"/>
                    <p id="abl-viewing-user">Username</p>

                    <div>
                        <button id="abl-ownerEditBtn" class="abl-ghost-button abl-centered abl-icon--md">{SVG(editSVG, "var(--color-content-default)", 24)}</button>
                        <div class="abl-background abl-container-list abl-big-dropdown abl-gap-sm" id="abl-ownerEditDropdown" style="display: none;">
                            <input type="text" id="abl-ownerEditDropdownSearch" class="abl-background abl-pad-sm" placeholder="Add username"/>
                            <div id="abl-ownerEditDropdownList" class="abl-container abl-column abl-gap-sm"></div>
                        </div>
                    </div>

                    <button id="abl-ownerRefreshBtn" class="abl-ghost-button abl-centered abl-icon--md">{SVG(refreshSVG, "var(--color-content-default)", 24)}</button>
                    <p id="abl-ownedRemaining"></p>
                    <p class="" id="abl-cloudKeyWarning" style="display: none;">⚠ Add a Cloud Key in ABL Settings to fetch more badges at once.</p>
                </div>

                <div class="abl-container abl-column" id="abl-filters-container">
                    <h4 id="abl-filters-title">Filters</h4>
                    <div class="abl-container">
                        <div id="abl-filter-options-list" class="abl-container-list abl-background"></div>
                        <div id="abl-filter-list" class="abl-container-list"></div>
                    </div>
                    <div class="abl-container abl-row">
                        <p>Sort by</p>
                        {createBasicDropdown(dropdownGroups.SortTypes, "abl-sortType")}
                        {createBasicDropdown(dropdownGroups.SortDirection, "abl-sortDirection")}
                    </div>
                </div>

                <div id="abl-status-container">
                    <p id="abl-status">Status</p>
                </div>

                <div class="abl-container abl-row abl-centered" id="abl-pages-container">
                    <button class="abl-background abl-button" id="abl-previousPage">&lt;</button>
                    <h2 class="abl-page-display" id="abl-pageDisplay">0</h2>
                    <button class="abl-background abl-button" id="abl-nextPage">&gt;</button>
                </div>

                <ul class="abl-container abl-column" id="abl-list">

                </ul>

                <div class="abl-centered" id="abl-bottom-container">
                    <button class="abl-background abl-button" id="abl-toTop">Back to Top</button>
                </div>
            </div>
        )
    }
    ...
    function createStatTemplate(title, text) {
        return (
            <div class="abl-container abl-row abl-container-stat">
                <p>{title}</p>
                <p class="abl-bold-text">{text}</p>
            </div>
        )
    };
    ...
    function createBadgeTemplate(id, value, imageSource, nameTxt, descTxt, awardedTotal, awardedToday, rate, active, owned) {
        ...
        const item = (<li className={`abl-background abl-container abl-container-badge-item ${!owned ? "abl-unowned" : ""}`}>
            <div>
                <a href={badgeURL}>
                    <img src={imageSource} alt="" className={`abl-badge-icon abl-icon--full ${value > 0 ? `abl-value-border ${valueName}` : ""}`}/>
                </a>
            </div>
            ...
            <ul class="abl-container-badge-stats">
                ...
                {createStatTemplate("Awarded Today", awardedToday)}
                {createStatTemplate("Rate", (Math.round(rate * 1000) / 10) + "%")}
            </ul>
        </li>)
    ...
    function createFilterOption(text) {
        return (
            <button class="abl-ghost-button abl--filter-option">
                {text}
            </button>
        )
    }
    ...
    function createFilterBlock(layout) {
        ...
        const deleteBtn = <button class="abl-ghost-button abl-centered abl-icon--md abl-deleteFilterBtn">
            {SVG(crossSVG, "var(--color-content-emphasis)", 18)}
        </button>
    ...
    function createOwnerEditOption(imageUrl, username) {
        return (<button class="abl-ghost-button abl-container abl-row abl--player-option">
            <img src={imageUrl} alt="" class="abl-icon abl-icon--full"/>
            <p>{username}</p>
        </button>)
    }
    ...
    function sortBadgeListArray(array, badgeList, keyName, sortDirection) {
        array.sort(function(a, b) {
            let resultA = badgeList.getBadgeInfo(a)[keyName];
            let resultB = badgeList.getBadgeInfo(b)[keyName];
            ...
            return sortDirection == "ascending" ? resultA - resultB : resultB - resultA;
        });
    }
    ...
    class FilteredBadgeList extends BadgeList {
        ...
        sortBadgeList(type, sortDirection) {
            const keys = {
                "awarded total": "count",
            ...
            if (keyName) {
                this.filteredList.sort((a, b) => {
                    ...
                    if (keyName == "award date") {
                        const nullifyDate = sortDirection == "ascending" ? "+275760-09-13T00:00:00.000Z" : "1970-01-01T00:00:00.000Z"
        ...
        refilterBadgeList() {
            ...
            for (const id of this.list) {
                ...
                const result = this.filterBadge({id: id, info: info});
    ...
    class OwnershipChecker {
        constructor(userId, badgeListArray, cloudKeyRotary) {
            this.userId = userId;
            this.badgeList = badgeListArray;
            this.cloudKeyRotary = cloudKeyRotary;

            this.currentIndex = 0;

            this.ownedBadges = new Set();
            this.unownedBadges = new Set();
            this.awardedDates = new Map();
        }
    ...
    class CentralOwnershipChecker {
        constructor(badgeListArray, cloudKeyRotary) {
            this.badgeList = badgeListArray;
            this.cloudKeyRotary = cloudKeyRotary;

            this.ownershipCheckers = new Map();
        }
    ...
    class UserInformation {
        ...
        async getInfoByUsername(username) {
            ...
            const infoResponse = await fetch("https://users.roblox.com/v1/usernames/users", {body: body, method: "POST"}).then(function(response) {
                return response.json();
            }).then(function(data) {
    ...
    async function fetchOwnershipByCloudKey(userId, badgeIds, cloudKey) {
        ...
        const response = await retry(async function() {
            return await fetch(URL, {headers: {"x-api-key": cloudKey}}).then(function(response){
                return response.json();
            }).then(function(data){
    ...
    const renderedBadgeNodes = new Map();
    const renderedBadgeImages = new Map();
    ...
    function refreshPage() {
        /*if (badgeList.filteredList.length < pageSize) {
